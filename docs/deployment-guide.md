...
### **服务器环境配置**
```bash
服务器类型: 共享主机 (cPanel/WHM)
SSH主机: www1099.onamae.ne.jp
SSH端口: 8022
SSH用户: r9048051
网站目录: public_html/daisoubus.jp/
...
2. 在服务器控制面板中：
   - 登录cPanel
   - 找到"SSH Access"或"SSH Keys"
   - 添加公钥内容

#### **1.3 测试SSH连接**
```powershell
# 测试连接
ssh -p 8022 r9048051@www1099.onamae.ne.jp

# 或指定密钥文件
ssh -p 8022 -i "$env:USERPROFILE\.ssh\id_rsa" r9048051@www1099.onamae.ne.jp
```
...
# 确认上传成功
ssh -p 8022 -i "$env:USERPROFILE\.ssh\id_rsa" r9048051@www1099.onamae.ne.jp "ls -la public_html/daisoubus.jp/index.html"
```
...
# 上传JS文件
scp -P 8022 -i "$env:USERPROFILE\.ssh\id_rsa" $jsFile.FullName r9048051@www1099.onamae.ne.jp:public_html/daisoubus.jp/assets/

# 上传CSS文件
scp -P 8022 -i "$env:USERPROFILE\.ssh\id_rsa" $cssFile.FullName r9048051@www1099.onamae.ne.jp:public_html/daisoubus.jp/assets/

# 批量上传所有assets（如果文件较多）
scp -P 8022 -r -i "$env:USERPROFILE\.ssh\id_rsa" dist/assets/* r9048051@www1099.onamae.ne.jp:public_html/daisoubus.jp/assets/
```

#### **6.3 上传其他资源文件**
```powershell
# 如果有新的图片或其他资源
scp -P 8022 -r -i "$env:USERPROFILE\.ssh\id_rsa" public/images/* r9048051@www1099.onamae.ne.jp:public_html/daisoubus.jp/images/

scp -P 8022 -r -i "$env:USERPROFILE\.ssh\id_rsa" public/icons/* r9048051@www1099.onamae.ne.jp:public_html/daisoubus.jp/icons/
```
...
SITE_DIR="$HOME/public_html/daisoubus.jp"
...
# 备份当前文件
tar -czf "current-backup-$(date +%Y%m%d-%H%M%S).tar.gz" -C ~/public_html/daisoubus.jp .
...
```powershell
# 检查连接参数
ssh -v -p 8022 r9048051@www1099.onamae.ne.jp

# 常见解决方案：
# 1. 确认主机名: www1099.onamae.ne.jp
# 2. 确认端口: 8022
# 3. 确认用户名: r9048051
# 4. 检查SSH密钥是否正确添加到服务器
```
...
```bash
# 检查文件权限
ssh -p 8022 r9048051@www1099.onamae.ne.jp "ls -la public_html/daisoubus.jp/assets/"

# 设置正确权限
ssh -p 8022 r9048051@www1099.onamae.ne.jp "chmod -R 644 public_html/daisoubus.jp/assets/*"
```

#### **问题5：文件上传失败**
```powershell
# 检查网络连接
Test-NetConnection -ComputerName www1099.onamae.ne.jp -Port 8022

# 检查SSH密钥权限
icacls "$env:USERPROFILE\.ssh\id_rsa"

# 使用详细模式调试
scp -v -P 8022 -i "$env:USERPROFILE\.ssh\id_rsa" dist/index.html r9048051@www1099.onamae.ne.jp:public_html/daisoubus.jp/
```
...
$server = "r9048051@www1099.onamae.ne.jp"
$remotePath = "public_html/daisoubus.jp"

# 上传主文件
scp -P 8022 -i $sshKey dist/index.html ${server}:${remotePath}/

# 上传资源文件
scp -P 8022 -r -i $sshKey dist/assets/* ${server}:${remotePath}/assets/
